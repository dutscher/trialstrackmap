angular.module("trialsTrackmap")
    .controller("trackMap", function (improveTimes, categories, urlHandler, localStorageService, trackData,
        ...
        function setWorldData() {
            ...
            $scope.worldImage = $filter("hoster")(activeWorld[isOffline ? "offline_src" : "src"], $scope.data.hoster);
        ...
        $rootScope.$on("data:loaded", function () {
            ...
            // set offline var
            isOffline = $scope.data.offline;

            setWorldData();

            // when track in url pan to them
            var trackParam = urlHandler.get("showTrack"),
                trData = trackParam ? trackParam.split("-") : [],
                trackId = trData[trData.length - 1],
                track;
        ...
        $rootScope.$on("filter:tracks", function (event, filterCatId) {
            ...
            if ($scope.tracksInActiveWorld === 0 && $scope.tracksInOtherWorld > 0) {
                $scope.changeWorld(trackData.getOtherWorld());
            }
        });
