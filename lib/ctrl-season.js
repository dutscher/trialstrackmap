            ...
            $scope.filteredArray.forEach(function (season, seasonIndex) {
                ...
                season.prizes.forEach(function (level, levelIndex) {
                    ...
                    if ('extra' in level) {
                        ...
                        // concat prizes
                        var foundDonkeyIndex = -1,
                            foundDonkey = season.summary.extras.filter(function (extra, index) {
                                var bool = startsWith(extra.css_selector,'donkey');
                                if(bool)
                                    foundDonkeyIndex = index;
                            return bool;
                        });
                        if (startsWith(level.extra_type,'donkey') && foundDonkey.length > 0) {
                            season.summary.extras[foundDonkeyIndex].amount += 1;
                        } else {
