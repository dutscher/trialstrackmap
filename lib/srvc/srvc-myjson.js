        ...
        return {
                        ...
                        if (response.status === 201) {
                            if ("uri" in response.data) {
                                ...
                                alert($filter("translate")(
                                    "page.timesTable.improve.successfullSaved", {id: id}));
                                return true;
                            }
                        }
                        return $q.reject();
            ...
            save: function (data, userName, password, autoExport) {
                ...
                if (this.getId() === null) {
                    ...
                    savePromise = this.put(url + "/" + this.getId(), data).then(
                        function (response) {
                            if (response.status === 200) {
                                return $q.resolve("successfullUpdated",
                                    {id: thisScope.getId()});
                            }
                            return $q.reject();
                        }
                ...
                return savePromise.then(function (saveBackup) {
                    ...
                    return getIDsPromise.then(function (response) {
                        ...
                        if (!(userName in listOfIds)) {
                            ...
                            // save
                            return thisScope.put(url + "/" + idStorageID, listOfIds);
                        }
                        return true;
                    });
                });
            ...
            getWRData: function () {
                if (cachePromise.wrData === null) {
                    var _url_ = !$useLocalJsonFiles
                        ? url + "/" + worldRecordsID
                        : $basePath + $lastBackupWRJson;

                    cachePromise.wrData = $http.get(_url_).then(function (response) {
                        return response.data;
                    });
                }
            ...
            setWRData: function (data) {
                return this.put(url + "/" + worldRecordsID, data);
            },

            getTags: function () {
                if (cachePromise.tagData === null) {
                    var _url_ = !$useLocalJsonFiles
                        ? url + "/" + tagsStorageID
                        : $basePath + $lastBackupWRJson;

                    cachePromise.tagData = $http.get(_url_).then(function (response) {
                        return response.data;
                    });
                }
                return cachePromise.tagData;
            },

            setTagsData: function (data) {
                return this.put(url + "/" + tagsStorageID, data);
            },

            put: function (url, data) {
                if ($useLocalJsonFiles) {
                    alert("at the moment is no saving possible!", url);
                    return $q.resolve({
                        status: 200
                    });
                }
