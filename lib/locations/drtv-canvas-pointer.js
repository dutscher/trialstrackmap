angular.module("trialsTrackmap")
    .directive("canvasPointer", function ($rootScope, $filter, $parse, $window, debounce, localStorageService) {
            ...
            link: function (scope, _canvas_, attrs) {
                ...
                if (onlyDimensions) {
                    $rootScope.$on("tracks:ready", function (event, data) {
                        ...
                        var ls = localStorageService.get(lsKey);
                        if (ls.world === world) {
                            $window.scrollTo(ls.x, ls.y);
                        }
                    });
                    ...
                    _canvas_.bind("mousedown", function (e) {
                        var coordX = parseInt(e.offsetX),
                            coordY = parseInt(e.offsetY);
                        $rootScope.$broadcast("copyText", coordX + "," + coordY);
                    });

                    window.on("scroll", debounce(function () {
                        localStorageService.set(lsKey, {x: $window.pageXOffset, y: $window.pageYOffset, world: world});
                    }, 250));
