angular.module("trialsTrackmap")
    .service("improveTimes", function (localStorageService, myJson, security, WRTimes,
        ...
        var thisScope = {
            ...
            getUlitmateData: function (trackData, myTime, _myFaults_) {
                // prepare the emptiness
                if (!myTime) {
                    return "";
                }

                var silver = trackData.times.silver.time,
                    gold = trackData.times.gold.time,
                    plat = trackData.times.platinum.time,
                    ultimate = plat / 2,
                    medalRange = silver - plat,
                    zeroFaultScore = 0.0,
                    myFaults = _myFaults_ || 0,
                    caseZeroFault = "",
                    adjustedScore,
                    d;
                ...
                } else if (myTime <= gold && myTime > plat) {
                    zeroFaultScore = 500000 + thisScope.progressPercent(gold, plat, myTime) * 250000;
                    caseZeroFault = "0-fault gold";
                } else if (myTime <= plat && myTime >= ultimate) {
                    zeroFaultScore = 750000 + thisScope.progressPercent(plat, ultimate, myTime) * 250000;
