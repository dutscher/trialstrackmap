            ...
            updateMore: function (tracksData) {
                ...
                trackIds.forEach(function (_trackId_) {
                    var data = thisScope.getOneByType("all", _trackId_), // get local state
                        ...
                        importDataBikeKeys = Object.keys(tracksData[_trackId_]);
                    // iterate the bikeTypes normal | donkey | crazy
                    importDataBikeKeys.forEach(function (_bikeType_) {
                        var importDataKeys = Object.keys(tracksData[_trackId_][_bikeType_]);
                        // iterate the types myTime | myRank | myBike
                        importDataKeys.forEach(function (_type_) {
                            // myGoal-android-normal
                            var type = _type_ + "-" + mode + "-" + _bikeType_,
                                _value_ = _type_ !== "myBike" ? clearTime(tracksData[_trackId_][_bikeType_][_type_]) : tracksData[_trackId_][_bikeType_][_type_];
                            data[pattern[type]] = _value_ !== 0 ? _value_ : "";
                        });
                    });
                    // save local
